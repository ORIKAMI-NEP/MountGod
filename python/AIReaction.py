...
def LearnAIReaction(reaction):
    returnValue = None
    if (
        str(reaction.emoji) in "<a:iikaeshi:889297289401761852>"
        or "!fl" in reaction.message.content
    ):
        reactionAlias = str(reaction.message.reactions[0])[
            2 : str(reaction.message.reactions[0]).rfind(":")
        ]
        with open(
            "json/DefaultReaction.json", "r", encoding="utf-8"
        ) as DefaultReaction:
            DefaultReaction = json.load(DefaultReaction)
            if reactionAlias not in DefaultReaction:
                return
            message = ExtractJapanese(
                reaction.message.content[reaction.message.content.rfind("\n") + 1 :]
            )
...
def ReturnAIReaction(message):
    ...
    if "!fl" not in message:
        ...
        if len(messageContent) <= 20 and len(messageContent) >= 5:
            with open(
                "json/LearnedData.json", "r", encoding="utf-8"
            ) as LearnedData, open(
                "json/DefaultReaction.json", "r", encoding="utf-8"
            ) as DefaultReaction:
                LearnedData = json.load(LearnedData)
                ...
                ApproximateWord = ["", 100]
                for data in LearnedData:
                    distance = Levenshtein.distance(message, data["message"])
