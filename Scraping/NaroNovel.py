...
with open("../../data/naroNovel.txt", "w", encoding="utf-8", newline="\n") as f:
    ...
    for i in tqdm(range(all_queue_cnt)):
        payload = {'out': 'json', 'order': 'hyoka', 'gzip': 5,
                   'opt': 'weekly', 'lim': 500, 'lastup': "1073779200-"+str(lastup)}
        ...
        lastup = datetime.datetime.strptime(
            last_general_lastup, "%Y-%m-%d %H:%M:%S").timestamp()
        lastup = int(lastup)
        # 取得間隔を空ける
        tm.sleep(interval)
    ...
    for i in tqdm(range(1, len(df))):
        ...
        info_url = "https://ncode.syosetu.com/novelview/infotop/ncode/{}/".format(
            ncode)

        # info_res = request.urlopen(info_url)
        while(1):
            try:
                info_res = request.urlopen(info_url, timeout=10)
                break
            except:
                print("a")
                # logging.error(
        ...
        for part in range(1, num_parts+1):
            ...
            print("part {:d} downloaded (total: {:d} parts)".format(
                part, num_parts))  # 進捗を表示
            time.sleep(0.001)
        ncode = ""
